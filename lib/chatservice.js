...
Server.prototype.start = function(app) {
  ...
  this.server.set("close timeout", 20);
  this.server.set("heartbeat timeout", 20);
  this.server.set("heartbeat interval", 15);
      ...
      client.auth(function(err) {
        ...
        ClientContainer.clients.forEach(function(c) {
          if (new RegExp("_" + id + "$").test(c.hash)) {
            ...
            if (c.faction !== null) {
              c.stream.leave(c.faction);
              S.sockets["in"](c.faction).emit('client-count', {
                count: S.sockets.clients(c.faction).length,
                channel: "faction"
              });
            }
          }
